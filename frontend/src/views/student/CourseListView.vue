...
<template>
  <div class="page-view">
    <PageHero
      title="课程列表"
      description="学生可在本页面查看开放课程、搜索课程信息，并完成在线选课与退课。"
      tag="学生端"
    >
    ...
    <div class="card-grid">
      ...
      <StatCard title="开放课程" :value="stats.openCount" desc="当前页允许选课的课程数量" theme="emerald" />
      <StatCard title="已选课程" :value="stats.selectedCount" desc="当前页中你已选中的课程" theme="amber" />
      <StatCard title="平均余量" :value="stats.remainAvg" desc="当前页课程平均剩余名额" theme="violet" />
    </div>
    ...
    <section class="panel panel-inner" v-loading="loading">
      ...
      <EmptyState v-else title="没有匹配课程" description="可调整筛选条件或确认后端初始化数据已导入。" />
    </section>

    <el-drawer v-model="detailVisible" title="课程详情" size="480px">
      <div v-loading="detailLoading">
        <template v-if="detail">
          ...
          <div class="card-grid" style="margin-top: 18px">
            ...
            <StatCard title="平均成绩" :value="detail.score_stats?.avg_score ?? '--'" desc="若成绩尚未录入则显示为空" theme="amber" />
          </div>
        </template>
      </div>
    </el-drawer>
